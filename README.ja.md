...
## 状態: 現行の ComfyUI では基本的に不要です

ComfyUI 本体が 2026-08-12 に「ブロックを追加した Anima 派生モデル」のネイティブ対応をマージしました（[PR #15555](https://github.com/Comfy-Org/ComfyUI/pull/15555)）。ブロック数を state_dict から算出する修正です。**その commit を含む ComfyUI なら、本パッチを入れる代わりに本体を更新してください。**

古い ComfyUI に固定している場合は引き続き有用です。またロード後検証には上流に相当機能がありません（切り詰められたモデルを通さずロードを明示的に失敗させる）。Anima 系チェックポイントを自分で作成・拡張する場合には役立ちます。
